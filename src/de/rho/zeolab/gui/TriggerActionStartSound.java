...
public class TriggerActionStartSound implements IZeoTriggerAction, Serializable
{
	@Override
	public void actionTriggered(String wavfile) 
	{
		try 
		{
			Date now = new Date();
			System.out.println(now.toString() + ": play wavfile " + wavfile);
			WavPlayer.play(wavfile);
			//wavPlayer.start(wavfile);
